#include "UNO_Obloq2.h"

/*
void UNO_Obloq2::registerDubugHandle(const MsgHandleDebug handle)
{
    if(!handle) return;
    debugHandle = handle;
}
*/
void UNO_Obloq2::startConnect(uint8_t receive,
                             uint8_t send,
                             const String &ssid,
                             const String &pwd,
                             const String &iotId,
...
void UNO_Obloq2::startConnect(Stream *serial,
                             const String &ssid,
                             const String &pwd,
                             const String &iotId,
                             const String &iotPwd,
...
void UNO_Obloq2::publish(Topicnum topic_num, const String &message)
{
...
void UNO_Obloq2::publish(Topicnum topic_num, double f, int precision)
{
    if (topic_num >= mqttTopicArray.size() || currentStatus != OBLOQ_SUB_OK)
        return;
...
void UNO_Obloq2::publish(Topicnum topic_num, int32_t i)
{
    if (topic_num >= mqttTopicArray.size() || currentStatus != OBLOQ_SUB_OK)
        return;
    char buffer[34];
...
String UNO_Obloq2::getMessage(Topicnum topic_num)
{
    obloqTick();
...
bool UNO_Obloq2::available(Topicnum topic_num)
{
    obloqTick();
...
String UNO_Obloq2::get(const String &url, int timeOut)
{
    OBLOQ_MESSAGE = "";
...
String UNO_Obloq2::post(const String &url, const String &content, int timeOut)
{
    OBLOQ_MESSAGE = "";
    if (currentStatus != OBLOQ_WIFI_CONNECT_OK)
        return OBLOQ_MESSAGE;
    //sendMessage(/*"Http post", */ "|3|2|http://" + http_host + "/" + url + "," + content + "|\r");
    sendMessage(/*"Http post", */ "|3|2|http://" + http_host + "/" + url  + "|\r");//obloq不支持https
...
String UNO_Obloq2::put(const String &url, const String &content, int timeOut)
{
    OBLOQ_MESSAGE = "";
    if (currentStatus != OBLOQ_WIFI_CONNECT_OK)
        return OBLOQ_MESSAGE;
...
UNO_Obloq2::UNO_Obloq2() : IPAddress("0.0.0.0")
{
    parameteReset();
}
UNO_Obloq2::~UNO_Obloq2()
{
    parameteReset();
}

ObloqErrorCode UNO_Obloq2::ping()
{
...
ObloqErrorCode UNO_Obloq2::wifiConnect(const String &ssid, const String &pwd)
{
...
ObloqErrorCode UNO_Obloq2::mqttConnect(const String &iotId,
                                      const String &iotPwd,
                                      const String iotTopics[],
                                      const String &server,
                                      uint32_t port)
...
ObloqErrorCode UNO_Obloq2::subTopic()
{
...
void UNO_Obloq2::sendMessage(/*const String& steps, */ String message)
{
    OBLOQ_CMD = OBLOQ_NONE;
...
/*
void UNO_Obloq2::debugPeek(const String& debugMessage, int8_t errorCode){
    if(debugHandle)
        debugHandle(debugMessage, errorCode);
}
void UNO_Obloq2::debugTask()
{
    switch(currentError){
